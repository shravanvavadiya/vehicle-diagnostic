...
class _QuestionAndAnsScreenState extends State<QuestionAndAnsScreen> {
  ...
  Widget build(BuildContext context) {
    return GetX<QuestionAndAnsController>(
      ...
      builder: (controller) => Scaffold(
          ...
          body: Column(
            children: [
              _buildLinearProgress(controller),
              _buildQueDataPageView(controller),
            ],
          ),
          floatingActionButtonLocation:
              FloatingActionButtonLocation.endContained,
          floatingActionButton: _buildNextSubmitBtn(controller)),
    );
  }

  Widget _buildNextSubmitBtn(QuestionAndAnsController controller) {
    return controller.isResponseData.value == true
        ? GestureDetector(
            onTap: controller.currentStep.value <
                        controller
                                .vehicleModel.value.apiresponse!.data!.length -
                            1 &&
                    controller.selectedAnswers
                        .containsKey(controller.currentStep.value)
                ? () {
                ...
                : controller.currentStep.value ==
                            controller.vehicleModel.value.apiresponse!.data!
                                    .length -
                                1 &&
                        controller.selectedAnswers
                            .containsKey(controller.currentStep.value)
                    ? () {
                        ...
                        widget.screenName == AppString.editScreenFlag
                            ? controller.EditForm(
                                ...
                                selectedResponse:
                                    controller.selectedResponseList)
  ...
  Widget _buildQueDataPageView(QuestionAndAnsController controller) {
    return controller.isResponseData.value == true
        ? Expanded(
            child: PageView.builder(
              ...
              itemCount:
                  controller.vehicleModel.value.apiresponse!.data!.length,
              itemBuilder: (context, index) {
                return _buildStepPage(
                    controller.vehicleModel.value.apiresponse!.data![index]
                        .question!.obs,
                    controller
                        .vehicleModel.value.apiresponse!.data![index].answers!,
                    controller
                        .vehicleModel.value.apiresponse!.data![index].key!.obs,
                    index.obs,
                    controller);
              },
            ),
  ...
  Widget _buildLinearProgress(QuestionAndAnsController controller) {
    return Padding(
  ...
  AppBar _buildAppBar(QuestionAndAnsController controller) {
    return AppBar(
            ...
            child: Text.rich(
              TextSpan(
                children: [
                  ...
                  TextSpan(
                    text:
                        "/${controller.vehicleModel.value.apiresponse?.data?.length ?? 0} ${AppString.steps}",
                    style: TextStyle(
  ...
  Widget _buildStepPage(RxString question, List<dynamic> answers,
      RxString questionKey, RxInt index, QuestionAndAnsController controller) {
    String? selectedAnswer = controller
        .selectedAnswers[index.value]; // Get the selected answer for this page
                      ...
                      onChanged: (bool? value) {
                        ...
                        if (value == true) {
                          ...
                          controller.selectedAnswers[index.value] =
                              answers[listIndex]; // Store answer by index
                          ...
                          controller.selectedResponseList.removeWhere(
                              (element) =>
                                  element.question == questionKey.value);

                          controller.selectedResponseList
                              .add(SelectedQnsAnsModel(
